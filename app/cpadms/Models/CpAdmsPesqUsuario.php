...
class CpAdmsPesqUsuario {
    ...
    private function pesqUsuarioComp() {
        ...
        $paginacao->paginacao("SELECT COUNT(user.id) AS num_result 
                ...
                WHERE nivac.ordem >=:ordem AND user.nome LIKE '%' :nome '%' AND user.email LIKE '%' :email '%' ", "ordem=" . $_SESSION['ordem_nivac'] . "&nome={$this->Dados['nome']}&email={$this->Dados['email']}");
        ...
        $listarUsuario->fullRead("SELECT user.id, user.nome, user.email,
                ...
                INNER JOIN adms_niveis_acessos nivac ON nivac.id=user.adms_niveis_acesso_id
                WHERE nivac.ordem >=:ordem AND (user.nome LIKE '%' :nome '%' AND user.email LIKE '%' :email '%')
